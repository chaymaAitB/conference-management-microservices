...
*Note : La sécurité étant activée, il est nécessaire d'avoir un serveur Keycloak fonctionnel sur le port 8080 (ou mappé différemment) avec le Realm configuré comme indiqué dans `application.yml` et `auth.config.ts`.*

## 6. Dépannage

*   **Erreur "unable to prepare context" / Dockerfile manquant** : Les Dockerfiles ont été ajoutés à la racine de chaque micro-service. Assurez-vous d'avoir la dernière version du code.
*   **Lenteur du build Maven** : Le téléchargement des dépendances Spring Boot peut être long lors la première compilation. Patientez jusqu'au message "BUILD SUCCESS".
*   **Services non trouvés (Eureka)** : Attendre quelques secondes que tous les services s'enregistrent auprès d'Eureka après le démarrage.
